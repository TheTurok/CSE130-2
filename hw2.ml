...
let rec assoc (d,k,l) = match l with
  | [] -> d (* return d if it goes to the end or empty list *)
  | (name,number)::t -> if k = name then number  (*access tuple and see if it mathches name*)
      else assoc(d,k,t)
...
let removeDuplicates l = 
  let rec helper (seen,rest) = 
    match rest with 
        [] -> seen
      | h::t -> (* If the list seen has any h in it, it will return true making it equal seen*)
          let seen' = if List.mem h seen then seen else [h]@seen in (*if not add h to seen*)
          let rest' = t in (*recurse to the rest of t*)
            helper (seen',rest') 
  in
    List.rev (helper ([],l))
...
*)
let rec wwhile (f,b) = 
  let tuple = f b in (* let tuple = what the function returns*)
    match tuple with
      | (value, boolean) -> if boolean  (*access elements*)
          then wwhile(f, value) (*Recurse the function until it breaks bool*)
          else value
...
let fixpoint (f,b) = wwhile ((function value -> let b = (f value) in
                                 (b,b != value)) , b)
(*function to change b  and return false if b = x(new b) ending the function*)
...
type expr = 
    ...
    | Swag     of expr * expr * expr 
    | Fosho    of expr

(* exprToString : expr -> string
   Complete this function to convert an expr to a string 
*)
let rec exprToString e = match e with  (*translated given grammar to string*)
  | VarX		    -> "x" 
  | VarY		    -> "y"
  ...
  | Swag(a, b, c)     -> "cosh(pi*" ^ exprToString a ^ ")*sinh(pi*" ^ exprToString b ^ ")*tanh(pi*" ^ exprToString c ^ ")"
  | Fosho(a)          -> "tan(pi*" ^ exprToString a ^ ")"
...
let buildSwag(e1, e2, e3)          = Swag(e1, e3, e3)
let buildFosho(e1)	           = Fosho(e1)
...
let rec eval (e,x,y) = 
  match e with (*wrote the math recursively for the expr*)
    | VarX               -> x
    | VarY		     -> y
    ...
    | Swag(a, b, c)      -> cosh(pi*. eval(a,x,y)) *. sinh(pi*. eval(a,x,y)) *. tanh(pi*. eval(a,x,y))
    | Fosho(a)           -> tan(pi*. eval(a, x, y))
...
let rec build (rand, depth) =
  let randomInt = rand (0,7) in (*random integer to choose a build*)
  let theDepth = depth - 1(*(randomInt mod 2)*) in

    if depth < 1 (*When depths is at its end, build X and Y*)
    then let randomInt = rand(0,1) in
        match randomInt with
          |0 -> buildX()
          |1 -> buildY()
    else
      match randomInt with
        |0 -> build(rand,theDepth)
        |1 -> buildSine(build(rand, theDepth))
        |2 -> buildCosine(build(rand, theDepth))
        |3 -> buildAverage(build(rand, theDepth),build(rand, theDepth))
        |4 -> buildTimes(build(rand, theDepth),build(rand, theDepth))
        |5 -> buildThresh(build(rand, theDepth),build(rand, theDepth),build(rand, theDepth), build(rand, theDepth))
        |6 -> buildSwag(build(rand, theDepth),build(rand, theDepth),build(rand, theDepth))
        |7 -> buildFosho(build(rand, theDepth)) 
...
(* doRandomGray : int * int * int -> unit
   Given a depth and two seeds for the eandom number generator,
